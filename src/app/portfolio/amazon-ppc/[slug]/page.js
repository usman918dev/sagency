...
export async function generateMetadata({ params }) {
  ...
  const pageUrl = `https://www.derixio.com/portfolio/amazon-ppc/${caseStudy.slug}`;
...
export default async function AmazonPpcCaseStudySlugPage({ params }) {
  const { slug } = await params;
  const caseStudy = await getAmazonPpcCaseStudyBySlug(slug);
  ...
  const pageUrl = `/portfolio/amazon-ppc/${caseStudy.slug}`;
